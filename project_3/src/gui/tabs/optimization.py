"""
Optimization Tab:
Maximize mean lift coefficient (Cl)
"""
import dearpygui.dearpygui as dpg

from gui.airfoil_canvas import render_geometry
from optimization.optimizer import OptimisationRun, ProgressUpdate, OptimisationResult
...
def build(parent_tag: str, designs: list):
    ...
    with dpg.tab(label="Optimization", parent=parent_tag, tag="tab_optimization"):
...
def _build_top_bar(designs):
    ...
    # Settings for performance (number of alpha values)
    with dpg.group(horizontal=True):
        with dpg.group():
            dpg.add_text("Simulation Settings", color=(200, 200, 200))
            dpg.add_separator()
            dpg.add_spacer(height=4)
            dpg.add_slider_int(
                label="Alpha Samples (0°-10°)",
                tag="opt_alpha_samples",
                default_value=3,
                min_value=2,
                max_value=11,
                width=250,
                user_data="Controls simulation fidelity vs speed (e.g. 3 samples = 0°, 5°, 10°)"
            )

        dpg.add_spacer(width=40)

        with dpg.group():
            dpg.add_text("Objective", color=(200, 200, 200))
            dpg.add_separator()
            dpg.add_spacer(height=4)
            dpg.add_combo(
                items=["Maximize Mean Cl", "Maximize Min Cl"],
                default_value="Maximize Mean Cl",
                tag="opt_objective_target",
                width=180
            )
...
def _build_loss_plot():
    dpg.add_text("Objective Fitness over iterations (Negative Loss)", color=(200, 200, 200))
    with dpg.plot(height=200, width=-1):
        dpg.add_plot_legend()
        dpg.add_plot_axis(dpg.mvXAxis, label="Iteration", tag=_TAG_LOSS_AXIS_X)
        dpg.add_plot_axis(dpg.mvYAxis, label="Fitness (-Loss)", tag=_TAG_LOSS_AXIS_Y)
        dpg.add_line_series([], [], label="Best Fitness",
...
def _build_best_result_panel():
    ...
    with dpg.group(horizontal=True):
        with dpg.group():
            dpg.add_text("Aerodynamic Metrics", color=(180, 180, 180))
            dpg.add_spacer(height=4)
            for label, tag in [
                ("Best Mean Cl", "opt_best_loss"),
                ("Min Cl (0°)", "opt_best_min_cl"),
                ("Max Cl (10°)", "opt_best_max_cl"),
                ("Max Thickness", "opt_best_thickness"),
                ("Max Camber", "opt_best_camber"),
        ...
        with dpg.group():
            dpg.add_text("Optimal Parameters", color=(180, 180, 180))
            dpg.add_spacer(height=4)
            dpg.add_group(tag="opt_best_params_group")

    dpg.add_spacer(height=14)
    dpg.add_text("Airfoil live preview (shown when run completes)", color=(180, 180, 180))
    dpg.add_spacer(height=4)
...
def _update_best_panel(progress: ProgressUpdate):
    ...
    feat_color = (100, 220, 100) if feasible else (255, 100, 100)

    dpg.set_value("opt_best_loss", f"{r.get('mean_cl', 0.0):.4f}")
    dpg.set_value("opt_best_min_cl", f"{r.get('min_cl', 0.0):.4f}")
    dpg.set_value("opt_best_max_cl", f"{r.get('max_cl', 0.0):.4f}")

    # plot the found solution
    dpg.set_value("opt_best_thickness", f"{progress.best_params.get('thickness', 0.0)*100:.1f} %")
    dpg.set_value("opt_best_camber", f"{progress.best_params.get('camber', 0.0)*100:.1f} %")

    dpg.set_value("opt_best_feasible", "yes" if feasible else "no")
    dpg.configure_item("opt_best_feasible", color=feat_color)
...
def _on_run_finished(result: OptimisationResult):
    ...
    if result.best_params is not None and _s.design is not None:
        try:
            airfoil = _s.design.build_airfoil(result.best_params)
            pixel_data = render_geometry(airfoil, CANVAS_W, CANVAS_H, alpha_deg=0.0)
...
def _reset_ui():
    ...
    for tag in ("opt_best_loss", "opt_best_min_cl", "opt_best_max_cl", 
                "opt_best_thickness", "opt_best_camber", "opt_best_feasible"):
...
def _sync_canvas_width():
    ...
    if dpg.does_item_exist("opt_canvas"):
        ...
        if abs(new_w - CANVAS_W) > 10:
            CANVAS_W = new_w
            dpg.delete_item(_s.texture_tag)
            _s.texture_tag = _create_texture()
            dpg.configure_item("opt_canvas", texture_tag=_s.texture_tag, width=CANVAS_W)
